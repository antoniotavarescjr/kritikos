"""
Google Cloud Storage Utilities - VERSÃO CORRIGIDA
Módulo para gerenciamento de arquivos no GCS com cache local, compressão e paginação correta
"""
...
class GCSManagerCorrigido:
    """
    Gerenciador de arquivos no Google Cloud Storage - VERSÃO CORRIGIDA
    Correção principal: paginação adequada no list_blobs()
    """
    ...
    def _initialize_client(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ GCS Manager Corrigido inicializado - Bucket: {self.bucket_name}")
            return True
    ...
    def list_blobs(self, prefix: str = "", max_results: Optional[int] = None) -> list:
        """
        Lista blobs no bucket com paginação CORRETA
        ...
        Returns:
            list: Lista de blobs (COM PAGINAÇÃO CORRETA)
        """
        if not self.is_available():
            return []
        ...
        try:
            ...
            while True:
                ...
                blobs = list(self.client.list_blobs(
                    ...
                    max_results=1000  # Limite alto para reduzir páginas
                ))
                
                all_blobs.extend(blobs)
                
                # Verificar se há próxima página
                if hasattr(blobs, 'next_page_token') and blobs.next_page_token:
                    page_token = blobs.next_page_token
                    logger.debug(f"   Próxima página encontrada, total parcial: {len(all_blobs)}")
                else:
                    break
            
            # Aplicar limite se especificado (para compatibilidade)
            if max_results and len(all_blobs) > max_results:
                all_blobs = all_blobs[:max_results]
            
            logger.info(f"✅ list_blobs() corrigido: {len(all_blobs)} arquivos encontrados para prefixo '{prefix}'")
            return all_blobs
...
# Função utilitária para criar instância GCS corrigida
def get_gcs_manager_corrigido() -> GCSManagerCorrigido:
    """
    Cria instância do GCS Manager corrigido usando variáveis de ambiente
    
    Returns:
        GCSManagerCorrigido: Instância configurada ou None se não configurado
    """
    ...
    return GCSManagerCorrigido(bucket_name, project_id)
